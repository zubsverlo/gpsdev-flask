...
def objects_one(object_id=None):
    if request.method == "GET":
        obj = (
            db_session.query(ObjectsSite).filter_by(object_id=object_id).first()
        )
        if not obj:
    ...
    if request.method == "PATCH":
        ...
        db_session.execute(
            update(ObjectsSite).filter_by(object_id=object_id).values(**obj)
        )
        db_session.commit()
    ...
    if request.method == "DELETE":
        db_session.execute(delete(ObjectsSite).filter_by(object_id=object_id))
        db_session.commit()
        return jsonify({}), 204
...
def forbid_statements(object_id):
    ...
    if obj["allow"]:
        message = (
            f"Выходы к ПСУ {obj_db.name} за {obj['date']} снова разрешены."
        )
        db_session.execute(
