...
class EmployeesSchema(Schema):
    ...
    def validate_name(self, name):
        if (
            g.get("name_id")
            and db_session.query(Employees)
            .filter_by(name=name)
        ...
        if (
            not g.get("name_id")
            and db_session.query(Employees).filter_by(name=name).first()
        ):
            raise ValidationError(f"{name} уже существует")
...
class ObjectSchema(Schema):
    ...
    apartment_number = fields.String(
        validate=validate.Length(
            max=50, error="Нужно уложиться в 50 символов для номера квартиры"
        )
    )
    personal_service_after_revision = fields.String(
        validate=validate.Length(max=70, error="Нужно уложиться в 70 символов")
    )
    division_name = fields.Pluck(
        DivisionSchema, "division", attribute="division_ref", dump_only=True
    )
    comment = fields.String(
        validate=validate.Length(
            max=200, error="Комментарий быть в пределах 200 символов"
        )
    )
    holiday_attend_needed = fields.Boolean(load_default=False)
    ...
    def validate_unique_name(self, data, **kwargs):
        ...
        if existing_objects:
            raise ValidationError("Уже используется в указанном подразделении")
...
class UserSchema(Schema):
    ...
    password = fields.String(
        validate=validate.Length(max=60), load_only=True, required=True
    )
    ...
    def load_password_and_access(self, data, **kwargs):
        ...
        if "access_set" in data.keys():
            ...
            res = (
                db_session.query(Division)
                .filter(
                    or_(Division.id.in_(access), Division.division.in_(access))
                )
                .all()
            )
            del data["access_set"]
    ...
    def dump_access(self, data, **kwargs):
        if data.get("access"):
            access = data["access"]
            access_processed = [
                {"division_id": i.id, "division": i.division} for i in access
            ]
...
class StatementsSchema(Schema):
    ...
    date = fields.Date(required=True)
    value = fields.String(
        required=True, validate=validate.OneOf(["", "В", "Б", "О", "У", "Н"])
    )
    ...
    def validate_value(self, data, **kwargs):
        if data["value"] == "В" and data["object_id"] == 1:
            raise ValidationError(
                "Нельзя проставить В для " "БОЛЬНИЧНЫЙ/ОТПУСК/УВОЛ."
            )
...
class ReportSchema(Schema):
    ...
    def validate_division(self, value):
        if value not in current_user.access_list:
            raise ValidationError(
                f"You should use actual division number or "
    ...
    def validate_params(self, data, **kwargs):
        if data.get("division") and (
            ...
            raise ValidationError(
                "Указывая список сотрудников или объектов, "
...
class MapMovementsSchema(Schema):
    ...
    def validate_date(self, value):
        if value > dt.date.today():
            raise ValidationError(
                "Вы пытаетесь запросить отчет за день, "
...
class FrequencySchema(Schema):
    ...
    frequency = fields.String(
        validate=validate.Length(
            max=250, error="Длина для кол-ва выходов не более 3 символов!"
        ),
        required=True,
        allow_none=True,
    )
...
class OwnTracksLocationSchema(Schema):
    ...
    tst = fields.DateTime(required=True)
    vel = fields.Integer()
