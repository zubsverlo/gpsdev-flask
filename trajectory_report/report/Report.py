...
class Report(ReportBase):
    ...
    def _build(self):
        ...
        statements_with_journal = (
            self._create_stmts_with_journal_j_exist_vector()
        )
        ...
        self.employees_subsid = stmts_jrnl_clstrs.drop_duplicates(
            ["name_id"]
        ...
        self.employees_to_notify: pd.DataFrame = (
            pd.merge(candidates_to_notify, self.employees_subsid, on="name_id")
    ...
    def _create_stmts_with_journal_j_exist_vector(self) -> pd.DataFrame:
        ...
        и создавать отчет, а в каком его нет.
        А также - какой subscriberID использовать."""
        stmts = pd.merge(self._stmts, self._journal, how="left", on="name_id")
    ...
    def _create_stmts_with_journal_j_exist(self) -> pd.DataFrame:
        ...
        # либо первый (без разницы, какой он там)
        stmts = stmts.groupby(
            by=["name_id", "object_id", "date"], as_index=False
    ...
    def _filter_serves(self, report: pd.DataFrame) -> pd.DataFrame:
        ...
        attendant_name_ids = self._schedules[
            self._schedules["schedule"] == 2
        ].name_id.unique()
    ...
    def _merge_into_one_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        mask_no_duration_but_approval = pd.isna(df.duration) & pd.notna(
            df.approval
        )
        # Нет отчета и нет служебки
        mask_no_duration_no_approval = pd.isna(df.duration) & pd.isna(
            df.approval
        )
        # Ещё не наступившие даты или "БОЛЬНИЧНЫЙ/ОТПУСК/УВОЛ."
        mask_future_or_first_object = (df.date > dt.date.today()) | (
            df.object_id == 1
        )
        ...
        if self._counts:
            df.loc[mask_attends_count, "result"] = df.loc[
                mask_attends_count, "attends_count"
            ].apply(lambda x: str(int(x)))
        else:
            df.loc[mask_duration, "result"] = df.loc[
...
class ReportWithAdditionalColumns(Report):
    ...
    def horizontal_report(self) -> pd.DataFrame:
        ...
        comments_and_frequency = pd.merge(
            self._comment,
            self._frequency,
