        ...
        with DB_ENGINE.connect() as conn:
            ...
            if includes_current_date:
                current_locations = pd.read_sql(
                    cs.current_locations(subs_ids), conn
                )
            ...
            holiday_attend_needed = [
                ...
                for i in conn.execute(
                    cs.holiday_attend_needed(object_ids)
                ).scalars()
            ]
...
def report_data_factory(
    date_from: Union[dt.date, str], *args, **kwargs
) -> dict:
    date_from = dt.date.fromisoformat(str(date_from))
...
if __name__ == "__main__":
    a = AsyncReportDataGetter().get_data(
        "2023-10-01", "2023-10-30", "ПВТ9,ПНИ25"
    )
    # print(a)
    pass
