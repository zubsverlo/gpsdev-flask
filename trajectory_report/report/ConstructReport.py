    ...
    def __get_divisions(self) -> dict:
        ...
        if not fetched:
            res = self._connection.execute(
                select(Division.id, Division.division)).all()
...
class AsyncReportDataGetter:
    ...
    async def trigger(self,
        ...
        tasks.extend([
            create_task(self.async_fetch(
                selects['schedules']), name='schedules'),
            create_task(self.async_fetch(selects['serves']), name='serves'),
            create_task(self.async_fetch(
                selects['clusters']), name='clusters'),
            create_task(self.async_fetch(selects['comment']), name='comment'),
            create_task(self.async_fetch(
                selects['frequency']), name='frequency')
        ]
        )
        if includes_current_date:
            tasks.append(
        ...
        results = {t.get_name(): pd.DataFrame(
            t.result(), columns=selects_cols[t.get_name()]) for t in tasks}
        pass
        ...
        with DB_ENGINE.connect() as conn:
            ...
            if includes_current_date:
                current_locations = pd.read_sql(
                    cs.current_locations_mts(subs_ids),
                    conn
                )
                current_locations['date'] = current_locations['locationDate'] \
                    .apply(lambda x: x.date())
...
class OneEmployeeDataGetterExtended:
    ...
    def get_data(self) -> dict:
        ...
        journal['period_end'] = journal['period_end'].fillna(dt.date.today())
        journal = journal \
            .loc[(journal['period_init'] <= self.date)
        ...
        locations = pd.read_sql(cs.locations_one_emp(self.date, subscriber_id),
                                self.conn)
        valid_locations = locations[pd.notna(locations['locationDate'])]
        if not len(locations) or not len(valid_locations):
            raise ReportException(
...
class OwntracksMtsReportDataGetter:
    ...
    def get_data(self) -> dict:
        ...
        serves = pd.read_sql(
            cs.serves(self.date_from, self.date_to, self.name_ids),
            self.conn
        )

        # Комментарии в таблице
        comment = pd.read_sql(
            cs.comment(self.division, self.name_ids),
            self.conn
        )
        # Частота посещений псу в таблице
        frequency = pd.read_sql(
            cs.frequency(self.division, self.name_ids),
            self.conn
        )
        ...
        self.journal['period_end'] = self.journal['period_end']\
            .fillna(dt.date.today())
        ...
        self.employees['empty_locations'] = self.employees\
            .name_id.isin(mts_empty_locations)
    ...
    def mts_clusters(self) -> pd.DataFrame:
        ...
        clusters = pd.read_sql(
            cs.clusters(self.date_from, self.date_to, subs_ids),
            self.conn
        )
        ...
        return clusters[
            ['name_id', 'date', 'datetime', 'leaving_datetime',
...
if __name__ == '__main__':
    ...
    #     "2024-02-01", "2024-02-27", 'ПВТ1').get_data()
